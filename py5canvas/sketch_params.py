...
class SketchParams:
    ...
    def save(self, path=""):
        ...
        data = {
            "params": filter_params(self.params),
...
if imgui is not None:
    ...
    def set_theme(hue=0.5):  # 0.127):
        ...
        imgui.style_colors_light(
            style
        )  # optional: set base colors from "Dark" (or any other) style
        # return
        ...
        col_main = imgui.color_convert_hsv_to_rgb(
            [hue, col_main_sat, col_main_val, 1.0]
        )[:3]
        col_back = imgui.color_convert_hsv_to_rgb(
            [hue, col_back_sat, col_back_val, 1.0]
        )[:3]
        col_area = imgui.color_convert_hsv_to_rgb(
            [hue, col_area_sat, col_area_val, 1.0]
        )[:3]
        ...
        style.colors[imgui.Col.TEXT] = [col_text[0], col_text[1], col_text[2], 1.00]
        ...
        style.colors[imgui.Col.BORDER] = [col_text[0], col_text[1], col_text[2], 0.30]
        style.colors[imgui.Col.BORDER_SHADOW] = [0.0, 0.0, 0.0, 0.0]
        style.colors[imgui.Col.FRAME_BG] = [col_area[0], col_area[1], col_area[2], 1.00]
        ...
        style.colors[imgui.Col.TITLE_BG] = [col_main[0], col_main[1], col_main[2], 0.45]
        ...
        style.colors[imgui.Col.BUTTON] = [col_main[0], col_main[1], col_main[2], 0.44]
        ...
        style.colors[imgui.Col.HEADER] = [col_main[0], col_main[1], col_main[2], 0.76]
        ...
        style.colors[imgui.Col.MODAL_WINDOW_DIM_BG] = [0.20, 0.20, 0.20, 0.35]
    ...
    class SketchGui:
            ...
            for name, val in gui_params.items():
                ...
                if type(val) == dict:  # A dict marks a group
                    ...
                    if imgui.tree_node(name, imgui.TreeNodeFlags.DEFAULT_OPEN):
                        imgui.push_id(name)
                        self.show_params(
                            params[key], val, parent + key + ".", depth + 1
                        )
                        imgui.pop_id()
                        imgui.tree_pop()
                else:
                    ...
                    try:
                        changed = False
                        if param_type == "int":
                            changed, params[key] = imgui.input_int(
                                name, int(params[key])
                            )
                            if "min" in opts:
                            ...
                            if "ret" in opts and not opts["ret"]:
                                flags = 0
                            if "buf_length" in opts:
                                buf_length = opts["buf_length"]
                            if "multiline" in opts and opts["multiline"]:
                                changed, params[key] = imgui.input_text_multiline(
                                    name, params[key], [0, 0]
                                )  # , imgui.InputTextFlags.ENTER_RETURNS_TRUE)
                            else:
                                changed, params[key] = imgui.input_text(
                                    name, params[key], flags
                                )
                        elif param_type == "selection":
                            changed, params[key] = imgui.combo(
                                name, params[key], opts["selection"]
                            )
                            ...
                            if len(clr) == 3:
                                changed, clr = imgui.color_edit3(
                                    name, clr
                                )  # , imgui.COLOR_EDIT_DISPLAY_HSV)
                            else:
                                changed, clr = imgui.color_edit4(
                                    name, clr
                                )  # , imgui.COLOR_EDIT_DISPLAY_HSV)
                            params[key] = (
                                np.array(clr)
                                * self.sketch.canvas.color_scale[: len(clr)]
                            )
                        if changed:
                            self.changed.add(parent + key)
                            if parent:
        ...
        def toolbar(self, sketch):
            ...
            if imgui.begin_popup("Save popup"):
                ...
                if imgui.menu_item("Pdf...")[0]:
                    path = sketch.save_file_dialog(
                        "pdf", filename=name
                    )  # ['pdf', 'svg', 'png'], filename=name)
            ...
            if imgui.begin_popup("Settings popup"):
                ...
                changed, flag = imgui.checkbox(
                    "Always on top", sketch.settings["floating_window"]
                )
                if changed:
                    sketch.set_floating(flag)
                changed, flag = imgui.checkbox(
                    "Startup with toolbar", sketch.settings["show_toolbar"]
                )
                if changed:
        ...
        def begin_gui(self, sketch):
            ...
            imgui.set_next_window_size(
                [self.width, (sketch.window_height - sketch.toolbar_height) * ratio]
            )
            imgui.set_next_window_pos(
                [(sketch.window_width - self.width) * ratio, sketch.toolbar_height]
            )
            imgui.begin(
                "Py5sketch",
                True,
        ...
        def show_sketch_controls(self):
            return imgui.collapsing_header(
                "Controls", None, imgui.TreeNodeFlags.DEFAULT_OPEN
            )[0]
        ...
        def from_params(self, sketch, callback=None, init=True):
            ...
            if imgui.collapsing_header(
                "Parameters", None, imgui.TreeNodeFlags.DEFAULT_OPEN
            )[0]:
            ...
            if sketch.params is not None:
                if imgui.collapsing_header(
                    "Presets", None, imgui.TreeNodeFlags.DEFAULT_OPEN
                ):
                    ...
                    if self.cur_preset_name:
                        ...
                        if imgui.button(label):
                            ...
                            preset_names = [k for k in sketch.params.presets.keys()]
                            sketch.current_preset = sketch.params.preset_index(
                                self.cur_preset_name
                            )
                    ...
                    if buttons:
                        imgui.same_line()
                    changed, self.cur_preset_name = imgui.input_text(
                        "Name",
                        self.cur_preset_name,
                        imgui.InputTextFlags.ENTER_RETURNS_TRUE,
                    )
                    ...
                    clicked, sketch.params.current_preset = imgui.list_box(
                        "Presets", sketch.params.current_preset, preset_names
                    )

                    if clicked:
                        sketch.params.apply_preset(
                            preset_names[sketch.params.current_preset]
                        )
                        self.cur_preset_name = preset_names[
                            sketch.params.current_preset
                        ]
                        self.force_changed(
                            sketch.params.params, sketch.params.gui_params
                        )

                    if changed:
                        if (
                            preset_name
                        ):  # self.cur_preset_name in sketch.params.presets:
                            sketch.params.presets[self.cur_preset_name] = (
                                sketch.params.presets.pop(preset_name)
                            )
                            sketch.params.current_preset = sketch.params.preset_index(
                                self.cur_preset_name
                            )
                            preset_names = [k for k in sketch.params.presets.keys()]
