...
class Renderer:
    ...
    def rectangle(self, x: float, y: float, width: float, height: float, radius: float):
        pass
    ...
    def select_font_face(
        self,
        family: str,
        slant: str = FontSlant.NORMAL,
        weight: str = FontWeight.NORMAL,
    ):
        pass

    def text_extents(self, text):
        pass  # returns TextExtents namedtuple
    ...
    def paint(self):
        pass  # fill whole canvas with current color
    ...
    def create_image_surface(self, width, height):
        return None
    ...
    def paint_image(self, img_surface, opacity=1.0):
        pass
...
try:
    ...
    class CairoRenderer(Renderer):
        ...
        def get_origin(self):
            ...
            return np.array([self._ctx.get_matrix().x0, self._ctx.get_matrix().y0])

        def get_line_width(self):
            return self._ctx.get_line_width()
        ...
        def set_line_width(self, w):
            self._ctx.set_line_width(w)

        def set_dash(self, dashes):
            self._ctx.set_dash(dashes)
        ...
        def select_font_face(
            ...
            self._ctx.select_font_face(
                family,
                _cairo_slant_map.get(slant, cairo.FontSlant.NORMAL),
...
class SVGRenderer(Renderer):
    ...
    def curve_to(self, cx1, cy1, cx2, cy2, x, y):
        self._add_path_command(
            f"C {cx1:.4f} {cy1:.4f} {cx2:.4f} {cy2:.4f} {x:.4f} {y:.4f}"
        )
    ...
    def select_font_face(
        self, family, slant=FontSlant.NORMAL, weight=FontWeight.NORMAL
    ):
    ...
    def paint(self):
        if self._background is not None:
            self._elements.insert(
                0, f'<rect width="100%" height="100%" fill="{self._background}" />'
            )
